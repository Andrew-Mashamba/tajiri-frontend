...
4. **Missing callbacks on child widgets** — Widgets like `PostCard`, `PostGridCell`, `CommentTile` rendered without required callbacks (`onLike`, `onComment`, `onShare`, `onSave`, `onUserTap`, `onHashtagTap`, `onMentionTap`, `onSubscribe`, `onMenuTap`, `onThreadTap`, `onReaction`) — any nullable callback left as `null` that should be wired up
