// FollowingManageScreen — owner-only management of the people the
// owner follows. Mirrors FollowersManageScreen with semantic swaps:
//   • "Remove follower" → "Unfollow"
//   • Insights pills relabeled (their data shape is identical)
//   • Visitor view continues to use ProfileStatsBottomSheet.
//
// Reuses FollowersInsightsCard and FollowerRow (the FollowUser model
// covers both surfaces). FollowerActionsSheet is parameterized so the
// "Remove" entry shows "Unfollow" here.
...
class FollowingManageScreen extends StatefulWidget {
  final int currentUserId;
  const FollowingManageScreen({super.key, required this.currentUserId});

  @override
  State<FollowingManageScreen> createState() => _FollowingManageScreenState();
}

class _FollowingManageScreenState extends State<FollowingManageScreen> {
  ...
  Future<void> _refreshAll() async {
    ...
    final results = await Future.wait([
      _service.getFollowingInsights(userId: widget.currentUserId),
      _service.getOwnerFollowing(
  ...
  Future<void> _appendNextPage() async {
    ...
    final next = _page + 1;
    final list = await _service.getOwnerFollowing(
      userId: widget.currentUserId,
      page: next,
      perPage: _perPage,
      q: _query.isEmpty ? null : _query,
  ...
  Future<void> _onLongPressRow(FollowUser f) async {
    ...
    final action = await FollowerActionsSheet.show(
      context,
      isMuted: f.isMuted,
      removeLabelEn: 'Unfollow',
      removeLabelSw: 'Acha kufuata',
    );
    ...
    switch (action) {
      ...
      case FollowerAction.remove:
        await _confirmAndUnfollow(f, isSw);
        break;
  ...
  Future<void> _confirmAndUnfollow(FollowUser f, bool isSw) async {
    final handle = (f.username ?? '').isNotEmpty ? '@${f.username}' : f.fullName;
    final ok = await _confirmDanger(
      title: isSw ? 'Acha kufuata' : 'Unfollow',
      body: isSw
          ? 'Acha kufuata $handle? Hawatajulishwa.'
          : "Unfollow $handle? They won't be notified.",
      confirmLabel: isSw ? 'Acha kufuata' : 'Unfollow',
    );
    if (!ok) return;
    await _service.unfollow(userId: widget.currentUserId, targetId: f.id);
    if (mounted) _refreshAll();
  }
  ...
  Future<void> _bulkConfirmUnfollow() async {
    final isSw = AppStringsScope.of(context)?.isSwahili ?? false;
    final n = _selected.length;
    final ok = await _confirmDanger(
      title: isSw ? 'Acha kufuata' : 'Unfollow',
      body: isSw
          ? 'Acha kufuata watu $n? Hawatajulishwa.'
          : "Unfollow $n people? They won't be notified.",
      confirmLabel: isSw ? 'Acha kufuata' : 'Unfollow',
    );
    if (!ok) return;
    HapticFeedback.mediumImpact();
    await _service.bulkUnfollow(
      userId: widget.currentUserId,
  ...
  Future<void> _bulkConfirmBlock() async {
    ...
    final ok = await _confirmDanger(
      title: isSw ? 'Zuia' : 'Block',
      body: isSw
          ? 'Zuia watu $n? Hawataweza kupata wasifu wako.'
          : "Block $n people? They won't be able to find your profile.",
      confirmLabel: isSw ? 'Zuia' : 'Block',
    );
    if (!ok) return;
    HapticFeedback.mediumImpact();
  ...
  Future<void> _onExport() async {
    ...
    final path =
        await _service.exportFollowingCsv(userId: widget.currentUserId);
    if (!mounted) return;
    ...
    await SharePlus.instance.share(
      ShareParams(
        files: [XFile(path)],
        text: isSw ? 'Ninaowafuata' : 'Following',
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: !_bulkMode
          ? null
          : _BulkActionBarFollowing(
              count: _selected.length,
              hint: _bulkHint,
              onUnfollow:
                  _selected.isEmpty ? null : _bulkConfirmUnfollow,
        ...
        children: [
          ...
          Text(
            filtered
                ...
                : (isSw ? 'Humfuati mtu yeyote bado' : 'Not following anyone yet'),
          ...
          Text(
            filtered
                ? ''
                : (isSw
                    ? 'Pata watu wa kuwafuata kwenye Discover'
...
class _BulkActionBarFollowing extends StatelessWidget {
  final int count;
  final String? hint;
                ...
                children: [
                  Expanded(
                    child: TextButton(
                      ...
                      child: Text(isSw ? 'Acha kufuata' : 'Unfollow'),
                    ),
                  ),
                  Expanded(
                    child: TextButton(
