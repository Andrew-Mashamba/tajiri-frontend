// FriendsManageScreen — owner-only management of accepted friendships.
// Mirrors FollowersManageScreen / FollowingManageScreen with semantic
// swaps: "Remove follower" → "Unfriend"; the 4th insights pill is
// repurposed to show *pending received friend requests* count and
// navigates to /requests instead of applying a filter.
//
// Reuses FollowersInsightsCard, FollowerRow, FollowerActionsSheet.
...
import '../followers/follower_actions_sheet.dart';
import '../followers/follower_row.dart';
import '../followers/followers_insights_card.dart';

class FriendsManageScreen extends StatefulWidget {
  final int currentUserId;
  const FriendsManageScreen({super.key, required this.currentUserId});
...
class _FriendsManageScreenState extends State<FriendsManageScreen> {
  ...
  Future<void> _refreshAll() async {
    ...
    final results = await Future.wait([
      _service.getFriendsInsights(),
      _service.getOwnerFriends(
  ...
  Future<void> _appendNextPage() async {
    ...
    final next = _page + 1;
    final list = await _service.getOwnerFriends(
      userId: widget.currentUserId,
      page: next,
      perPage: _perPage,
      q: _query.isEmpty ? null : _query,
  ...
  String? _filterToParam(FollowerFilter f) => switch (f) {
        ...
        // 4th pill repurposed as Pending — handled separately (navigates),
        // never sent as a filter.
        FollowerFilter.mutualGap => null,
      };
  ...
  void _onFilterTap(FollowerFilter f) {
    if (f == FollowerFilter.mutualGap) {
      // Pending pill — push to friend-requests screen instead of filter.
  ...
  Future<void> _onLongPressRow(FollowUser f) async {
    ...
    final isSw = AppStringsScope.of(context)?.isSwahili ?? false;
    final action = await FollowerActionsSheet.show(
      context,
      isMuted: f.isMuted,
      removeLabelEn: 'Unfriend',
      removeLabelSw: 'Ondoa urafiki',
    );
    ...
    switch (action) {
      ...
      case FollowerAction.remove:
        await _confirmAndUnfriend(f, isSw);
        break;
      case FollowerAction.block:
        await _confirmAndBlock(f, isSw);
        break;
    }
  }

  Future<void> _confirmAndUnfriend(FollowUser f, bool isSw) async {
    final handle = (f.username ?? '').isNotEmpty ? '@${f.username}' : f.fullName;
    final ok = await _confirmDanger(
      title: isSw ? 'Ondoa urafiki' : 'Unfriend',
      body: isSw
          ? 'Ondoa urafiki na $handle? Hawatajulishwa.'
          : "Unfriend $handle? They won't be notified.",
      confirmLabel: isSw ? 'Ondoa' : 'Unfriend',
    );
    if (!ok) return;
    await _service.removeFriend(widget.currentUserId, f.id);
    if (mounted) _refreshAll();
  }

  Future<void> _confirmAndBlock(FollowUser f, bool isSw) async {
    final handle = (f.username ?? '').isNotEmpty ? '@${f.username}' : f.fullName;
    final ok = await _confirmDanger(
      title: isSw ? 'Zuia' : 'Block',
      body: isSw
          ? 'Zuia $handle? Urafiki wenu utaondolewa. Hawataweza kupata wasifu wako.'
          : "Block $handle? Your friendship will be removed. They won't be able to find your profile.",
      confirmLabel: isSw ? 'Zuia' : 'Block',
    );
    if (!ok) return;
    HapticFeedback.mediumImpact();
    await _service.bulkBlockUsers(userId: widget.currentUserId, ids: [f.id]);
    if (mounted) _refreshAll();
  }
  ...
  Future<void> _bulkConfirmUnfriend() async {
    final isSw = AppStringsScope.of(context)?.isSwahili ?? false;
    final n = _selected.length;
    final ok = await _confirmDanger(
      title: isSw ? 'Ondoa urafiki' : 'Unfriend',
      body: isSw
          ? 'Ondoa urafiki na watu $n? Hawatajulishwa.'
          : "Unfriend $n people? They won't be notified.",
      confirmLabel: isSw ? 'Ondoa' : 'Unfriend',
    );
    if (!ok) return;
    HapticFeedback.mediumImpact();
    await _service.bulkUnfriend(ids: _selected.toList());
    if (!mounted) return;
    _exitBulkMode();
    await _refreshAll();
  }
  ...
  Future<void> _bulkConfirmMute() async {
    ...
    final ok = await _confirmDanger(
      ...
      body: isSw ? 'Nyamazisha watu $n?' : 'Mute $n people?',
  ...
  Future<void> _bulkConfirmBlock() async {
    ...
    final ok = await _confirmDanger(
      title: isSw ? 'Zuia' : 'Block',
      body: isSw
          ? 'Zuia watu $n? Urafiki utaondolewa.'
          : 'Block $n people? Friendships will be removed.',
      confirmLabel: isSw ? 'Zuia' : 'Block',
    );
    if (!ok) return;
    HapticFeedback.mediumImpact();
  ...
  Future<void> _onExport() async {
    ...
    final path = await _service.exportFriendsCsv();
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: !_bulkMode
          ? null
          : _BulkActionBarFriends(
              count: _selected.length,
              hint: _bulkHint,
              onUnfriend:
                  _selected.isEmpty ? null : _bulkConfirmUnfriend,
        ...
        children: [
          ...
          Text(
            filtered
                ...
                : (isSw ? 'Hujapata marafiki bado' : 'No friends yet'),
          ...
          Text(
            filtered
                ? ''
                : (isSw
                    ? 'Tuma maombi ya urafiki kupitia Discover'
                    : 'Send friend requests from Discover'),
...
class _BulkActionBarFriends extends StatelessWidget {
  final int count;
  final String? hint;
                ...
                children: [
                  Expanded(
                    child: TextButton(
                      ...
                      child: Text(isSw ? 'Ondoa' : 'Unfriend'),
                    ),
                  ),
                  Expanded(
                    child: TextButton(
