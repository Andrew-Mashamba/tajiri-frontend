// lib/rc/pages/rc_report_page.dart
...
class RcReportPage extends StatefulWidget {
  final int regionId;
  const RcReportPage({super.key, required this.regionId});

  @override
  State<RcReportPage> createState() => _RcReportPageState();
}

class _RcReportPageState extends State<RcReportPage> {
  ...
  Future<void> _submit() async {
    ...
    if (result.success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Ripoti imewasilishwa')),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Ripoti kwa RC',
      ...
      body: ListView(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ('general', 'Jumla'), ('infrastructure', 'Miundombinu'),
              ('education', 'Elimu'), ('health', 'Afya'),
              ('security', 'Usalama'),
            ].map((c) {
              final selected = c.$1 == _category;
              return ChoiceChip(
                label: Text(c.$2), selected: selected,
