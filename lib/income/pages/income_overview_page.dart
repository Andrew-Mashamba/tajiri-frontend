...
//
// Portfolio net income (collected invoice payments − expenses) across businesses.
...
import 'business_income_page.dart';
...
class _IncomeOverviewPageState extends State<IncomeOverviewPage> {
  ...
  PortfolioIncomeLoadResult? _result;
  bool _loading = true;
  String? _error;
  ...
  @override
  void didUpdateWidget(covariant IncomeOverviewPage oldWidget) {
  ...
  Future<void> _load() async {
    ...
    if (token == null) {
      ...
      setState(() {
        _loading = false;
        _error = _sw ? 'Ingia ili kuona faida' : 'Sign in to view income';
      });
      return;
    }
    try {
      final businesses = BusinessNotifier.instance.businesses;
      final result = await IncomeSummaryService.loadPortfolio(token, businesses, _scope);
      if (!mounted) return;
      setState(() {
        _result = result;
        _loading = false;
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                _sw
                                    ? 'Mapato yaliyokusanywa kutoka ankara (zilizohesabiwa) minus matumizi kwa kila biashara. Gusa biashara kwa maelezo na orodha ya miamala.'
                                    : 'Collected invoice payments (eligible invoices) minus expenses per business. Tap a business for detail and the transaction ledger.',
              ...
              SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                  child: Column(
                    ...
                    children: [
                      Text(
                        _sw ? 'Jumla (faida)' : 'Portfolio (net income)',
                        style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: _kPrimary),
                      ),
                      const SizedBox(height: 10),
                      _metricCard(
                        _sw ? 'Mapato yaliyokusanywa (ankara)' : 'Collected revenue',
                        _money(totals.collectedRevenue),
                      ),
                      const SizedBox(height: 10),
                      _metricCard(
                        _sw ? 'Jumla ya matumizi' : 'Total expenses',
                        _money(totals.totalExpenses),
                      ),
                      const SizedBox(height: 10),
                      _metricCard(
                        _sw ? 'Faida halisi' : 'Net income',
                        _money(totals.netIncome),
                      ),
              ...
              SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    ...
                    final highlight =
                        widget.featureBusinessId != null && row.businessId == widget.featureBusinessId;
                    return Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (row.loadFailed) ...[
                                  ...
                                  const SizedBox(height: 8),
                                  Text(
                                    '${_sw ? 'Faida halisi' : 'Net'} · ${_money(row.netIncome)}',
                                    style: const TextStyle(fontSize: 12, color: _kSecondary),
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                  ),
                                  Text(
                                    '${_sw ? 'Yaliyokusanywa' : 'Collected'} · ${_money(row.collectedRevenue)} · ${_sw ? 'Matumizi' : 'Expenses'} · ${_money(row.totalExpenses)}',
                                    style: const TextStyle(fontSize: 12, color: _kSecondary),
                                    maxLines: 2,
                                    overflow: TextOverflow.ellipsis,
                                  ),
