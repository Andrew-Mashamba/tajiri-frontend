...
import '../../escrow/widgets/escrow_status_card.dart';
import '../../escrow/screens/escrow_dispute_screen.dart';
...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Future<void> _loadEscrowInfo() async {
    try {
      final storage = await LocalStorageService.getInstance();
      final token = storage.getAuthToken();
      if (token == null || token.isEmpty) return;
      final info =
          await EscrowService.getOrderEscrow(widget.orderId, token);
      if (mounted && info != null) {
        setState(() => _escrowInfo = info);
      }
    } catch (_) {}
  }
  ...
  Future<void> _confirmReceived() async {
    ...
    setState(() => _isActioning = true);
    final result = await _repo.confirmReceived(
      widget.orderId,
      buyerId: widget.currentUserId,
    );

    if (result.success) {
      // Release escrow payment alongside order confirmation
      try {
        final storage = await LocalStorageService.getInstance();
        final token = storage.getAuthToken();
        if (token != null && token.isNotEmpty) {
          await EscrowService.releaseEscrow(widget.orderId, token);
        }
      } catch (_) {}
    }
    ...
    if (mounted) {
      ...
      messenger.showSnackBar(
        SnackBar(content: Text(result.success
            ? (s?.orderReceived ?? 'Order received — thank you!')
            : (result.message ?? (s?.failedToUpdateOrder ?? 'Failed to update order')))),
      );
      if (result.success) {
        _loadOrder();
  ...
  Future<void> _shipWithTracking() async {
    ...
    String selectedCarrier = 'Self-delivery';
    DateTime? eta;
    const carriers = ['Self-delivery', 'DHL', 'FedEx', 'Tanzania Post', 'Courier', 'Other'];
        ...
        builder: (ctx, setSheet) => Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Mark as Shipped', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: _kPrimaryText)),
              const SizedBox(height: 16),
              // Carrier chips
              SizedBox(
                height: 38,
                child: ListView(
                  ...
                  children: carriers.map((c) {
                    final sel = c == selectedCarrier;
                    return Padding(
                      padding: const EdgeInsets.only(right: 8),
                      child: GestureDetector(
                        onTap: () => setSheet(() => selectedCarrier = c),
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                          decoration: BoxDecoration(
                            color: sel ? _kPrimaryText : _kSurface,
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(color: sel ? _kPrimaryText : _kDivider),
                          ),
                          child: Text(c, style: TextStyle(fontSize: 13, color: sel ? Colors.white : _kPrimaryText)),
                        ),
                      ),
                    );
                  }).toList(),
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  if (picked != null) setSheet(() => eta = picked);
                },
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    Text(
                      eta != null ? 'Est. delivery: ${eta!.day}/${eta!.month}/${eta!.year}' : 'Set estimated delivery date (optional)',
                      style: TextStyle(fontSize: 13, color: eta != null ? _kPrimaryText : _kTertiaryText),
                    ),
                  ]),
                ),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.pop(ctx);
                    _updateStatus(OrderStatus.shipped, trackingNumber: trackingController.text.isNotEmpty ? trackingController.text : null);
                  ...
                  child: const Text('Mark as Shipped', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                ),
              ),
            ],
          ),
  ...
  Future<void> _showReturnDialog() async {
    ...
    setState(() => _isActioning = true);
    final result = await _repo.requestReturn(
      widget.orderId,
      buyerId: widget.currentUserId,
      reason: reason,
    );
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          // Escrow status
          if (!widget.isSeller && _escrowInfo != null) ...[
            const SizedBox(height: 16),
            EscrowStatusCard(escrow: _escrowInfo!),
          ],
  ...
  Widget _buildActions(BuildContext context, Order order) {
    ...
    if (widget.isSeller) {
      ...
      // Dispute button — show when escrow is held (raise) or already disputed (view)
      if (_escrowInfo?.status == EscrowStatus.held ||
          _escrowInfo?.status == EscrowStatus.disputed) {
        final hasDispute = _escrowInfo?.dispute != null ||
            _escrowInfo?.status == EscrowStatus.disputed;
        actions.add(_buildFullButton(
          label: hasDispute ? 'View Dispute' : 'Raise Dispute',
          icon: HeroIcons.exclamationTriangle,
          onTap: _navigateToDispute,
          isDestructive: !hasDispute,
        ));
      }
  ...
  Future<void> _navigateToDispute() async {
    try {
      final storage = await LocalStorageService.getInstance();
      final token = storage.getAuthToken();
      if (token == null || token.isEmpty) return;
      ...
      // Check if dispute already exists — if so, open DisputeDetailScreen.
      final existing = await EscrowService.getDispute(widget.orderId, token);
      if (!mounted) return;
      ...
      if (existing != null) {
        ...
        final result = await Navigator.push<bool>(
          context,
          MaterialPageRoute(
            builder: (_) => EscrowDisputeScreen(
              orderId: widget.orderId,
              token: token,
            ),
          ),
        );
        if (result == true) _loadEscrowInfo();
      }
    } catch (_) {}
  }
