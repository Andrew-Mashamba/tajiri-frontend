// lib/debts/pages/debts_page.dart
...
import '../../business/widgets/debt_card.dart';
...
class _DebtsPageState extends State<DebtsPage>
  ...
  Future<void> _syncDebtsFromCrb() async {
    if (_token == null) return;
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Sync from CRB?'),
        content: const Text(
          'Debts will be imported from your Credit Reference Bureau (Creditinfo) '
          'profile — the same data described in your credit report. '
          'TAJIRI requests this on the server; your phone is not given CRB login '
          'credentials. '
          'Service or inquiry fees may apply per Creditinfo policy.',
        ),
        actions: [
          TextButton(
    ...
    setState(() => _syncingCrb = true);
    try {
      final res =
          await BusinessService.syncDebtsFromCrb(_token!, widget.businessId);
      if (!mounted) return;
      setState(() => _syncingCrb = false);
            ...
            child: Row(
              children: [
                ...
                _topPillActionButton(
                  label: _syncingCrb ? 'Syncing…' : 'Sync CRB',
                  icon: Icons.cloud_download_rounded,
                  onTap: _syncingCrb ? null : _syncDebtsFromCrb,
                  filled: false,
                ),
                const SizedBox(width: 8),
                _topPillActionButton(
                          ...
                          ? Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  Text(
                                      'All clear! Use "New Debt" to track one.',
                                      style: TextStyle(
