// lib/revenue/pages/business_revenue_page.dart
//
// Single-business revenue from invoices + link to central ledger.
...
class BusinessRevenuePage extends StatefulWidget {
  ...
  @override
  State<BusinessRevenuePage> createState() => _BusinessRevenuePageState();
}

class _BusinessRevenuePageState extends State<BusinessRevenuePage> {
  final NumberFormat _nf = NumberFormat('#,###', 'en');

  RevenuePeriodScope _scope = RevenuePeriodScope.allTime;
  bool _loading = true;
  String? _loadError;
  RevenueTotals _totals = const RevenueTotals(gross: 0, collected: 0, outstanding: 0);
  RevenueLedgerHint? _ledgerHint;
  ...
  @override
  void didUpdateWidget(covariant BusinessRevenuePage oldWidget) {
  ...
  Future<void> _load() async {
    ...
    if (token == null) {
      ...
      setState(() {
        _loading = false;
        _loadError = _sw ? 'Ingia ili kuona mapato' : 'Sign in to view revenue';
      });
      return;
    }
    final out = await RevenueService.loadBusinessTotals(token, widget.businessId, _scope);
    if (!mounted) return;
    if (!out.success) {
      setState(() {
        _loading = false;
        _loadError = out.message ?? (_sw ? 'Imeshindwa kupakia mapato' : 'Failed to load revenue');
      });
      return;
    }
    setState(() {
      _totals = out.totals;
      _ledgerHint = out.ledgerHint;
      _loadError = null;
      _loading = false;
    });
  }
      ...
      child: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Text(
              _sw
                  ? 'Ankara zisizo rasimu/zilizofutwa ndizo zinahesabiwa. Kipindi kinategemea tarehe ya kuundwa.'
                  : 'Draft, void, and cancelled invoices are excluded. Period is based on invoice issue date.',
              style: TextStyle(fontSize: 11, color: Colors.grey.shade700, height: 1.3),
            ),
            const SizedBox(height: 16),
            _metricCard(_sw ? 'Mapato jumla (ankara)' : 'Gross billed', _money(_totals.gross)),
            const SizedBox(height: 12),
            _metricCard(_sw ? 'Yaliyokusanywa' : 'Collected', _money(_totals.collected)),
            const SizedBox(height: 12),
            _metricCard(_sw ? 'Baki' : 'Outstanding', _money(_totals.outstanding)),
            if (_ledgerHint != null) ...[
              const SizedBox(height: 12),
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        _sw
                            ? 'Orodha ya miamala (mapato yaliyofanikiwa, mwelekeo wa ndani): TZS ${_nf.format(_ledgerHint!.incomingSuccessTotal)} — mistari ${_ledgerHint!.rowCount}.'
                            : 'Ledger hint (successful incoming): TZS ${_nf.format(_ledgerHint!.incomingSuccessTotal)} across ${_ledgerHint!.rowCount} row(s). Uses completed_at on the server.',
