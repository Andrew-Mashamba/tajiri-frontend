// lib/my_children/pages/photo_journal_page.dart
...
class _PhotoJournalPageState extends State<PhotoJournalPage> {
  ...
  void _confirmDeletePhoto(BabyPhoto photo) {
    if (photo.id == null) return;
    final sw = _sw;
    final label = photo.caption ?? (sw ? 'Picha' : 'Photo');
    showDialog(
      context: context,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _service.deletePhoto(_token!, photo.id!);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
