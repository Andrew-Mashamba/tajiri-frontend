// lib/my_children/pages/sleep_tracker_page.dart
import 'dart:async';
...
class _SleepTrackerPageState extends State<SleepTrackerPage> {
  ...
  Future<void> _stopSleep() async {
    ...
    try {
      ...
      if (result.success) {
        _elapsedTimer?.cancel();
        // Schedule next nap reminder based on wake window
        final nextNapTime =
            DateTime.now().add(Duration(minutes: _wakeWindowMinutes()));
        ChildrenNotificationHelper.scheduleNapReminder(
          widget.baby.id,
          widget.baby.name,
          nextNapTime,
          _sw,
  ...
  void _confirmDeleteSleep(SleepSession session) {
    if (session.id == null) return;
    final sw = _sw;
    final label = session.type == 'nap'
        ? (sw ? 'Usingizi wa Mchana' : 'Nap')
        : (sw ? 'Usingizi wa Usiku' : 'Night sleep');
    showDialog(
      context: context,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _service.deleteSleep(_token!, session.id!);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  /// WHO recommended sleep hours by age (per 24h)
  double _recommendedHoursPerDay() {
    final months = widget.baby.ageInMonths;
    if (months < 4) return 16; // 0-3 months: 14-17h
    if (months < 12) return 14; // 4-11 months: 12-16h
    if (months < 36) return 13; // 1-2 years: 11-14h
    return 11; // 3-5 years: 10-13h
  }

  Widget _buildSleepQualityReport() {
    final sw = _sw;
    final data = _weeklyData();
    if (data.isEmpty) return const SizedBox.shrink();

    // Total weekly hours
    final totalWeekHours = data.values.fold<double>(0, (a, b) => a + b);
    final daysWithData = data.values.where((h) => h > 0).length;
    if (daysWithData == 0) return const SizedBox.shrink();

    final recommendedDaily = _recommendedHoursPerDay();
    final recommendedWeekly = recommendedDaily * 7;

    // Quality score (0-100)
    final qualityScore = ((totalWeekHours / recommendedWeekly) * 100)
        .clamp(0, 150)
        .round();

    // Longest stretch this week
    int longestStretchMin = 0;
    for (final s in _weekSessions) {
      final dur = s.endTime != null
          ? s.endTime!.difference(s.startTime).inMinutes
          : 0;
      if (dur > longestStretchMin) longestStretchMin = dur;
    }

    // Week-over-week comparison (simple: show vs recommended)
    final avgDailyHours = totalWeekHours / 7;
    final diffFromRecommended = avgDailyHours - recommendedDaily;
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  sw ? 'Ubora wa Usingizi' : 'Sleep Quality Report',
                  style: const TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.w600,
                    color: _kPrimary,
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      sw ? 'Alama ya Ubora' : 'Quality Score',
                      style:
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      sw ? 'Refu Zaidi' : 'Longest',
                      style:
                          const TextStyle(fontSize: 12, color: _kSecondary),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      _formatMinutes(longestStretchMin),
                      style: const TextStyle(
                        fontSize: 24,
                        fontWeight: FontWeight.w700,
                        color: _kPrimary,
          ...
          Column(
            ...
            children: [
              Text(
                sw
                    ? 'WHO inapendekeza: ${recommendedDaily.toStringAsFixed(0)}h/siku'
                    : 'WHO recommends: ${recommendedDaily.toStringAsFixed(0)}h/day',
                style: const TextStyle(fontSize: 12, color: _kSecondary),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
              ),
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: (avgDailyHours / recommendedDaily).clamp(0, 1),
              ...
              Row(
                children: [
                  ...
                  Text(
                    diffFromRecommended >= 0
                        ? (sw
                            ? '${diffFromRecommended.toStringAsFixed(1)}h zaidi ya pendekezo'
                            : '${diffFromRecommended.toStringAsFixed(1)}h above recommendation')
                        : (sw
                            ? '${(-diffFromRecommended).toStringAsFixed(1)}h chini ya pendekezo'
                            : '${(-diffFromRecommended).toStringAsFixed(1)}h below recommendation'),
                    style: TextStyle(
                      fontSize: 12,
                      color: diffFromRecommended >= 0
                          ? const Color(0xFF2E7D32)
  ...
  // ─── WHO minimum sleep hours by age (months) ────────────────
  double _whoMinSleepHours() {
    final months = widget.baby.ageInMonths;
    if (months < 4) return 14; // 0-3 months: 14-17h
    if (months < 12) return 12; // 4-11 months: 12-16h
    if (months < 36) return 11; // 1-2 years: 11-14h
    if (months < 72) return 10; // 3-5 years: 10-13h
    return 9; // 6+ years: 9-12h
  }

  bool get _showSleepConcernLink {
    // Show doctor link if total sleep today is below WHO minimum
    // (In real app, check 3+ days from _weekSessions)
    final weeklyData = _weeklyData();
    int lowDays = 0;
    final minHours = _whoMinSleepHours();
    for (final hours in weeklyData.values) {
      if (hours > 0 && hours < minHours) lowDays++;
    }
    return lowDays >= 3;
  }
  ...
  Widget _buildCrossModuleLinks() {
    ...
    final ageLabel = widget.baby.ageInMonths < 12
        ? (sw ? '${widget.baby.ageInMonths} miezi' : '${widget.baby.ageInMonths} months')
        : (sw ? '${widget.baby.ageInYears} miaka' : '${widget.baby.ageInYears} years');
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        if (_showSleepConcernLink && userId != null)
          _buildCrossModuleLink(
            icon: Icons.local_hospital_rounded,
            label: sw
                ? 'Usingizi mdogo sana - wasiliana na daktari'
        ...
        _buildCrossModuleLink(
          ...
          label: sw
              ? 'Uliza Shangazi kuhusu mafunzo ya usingizi kwa $ageLabel'
              : 'Ask Shangazi about sleep training for $ageLabel',
  ...
  Widget _buildSessionTile(SleepSession session) {
    ...
    return GestureDetector(
      onLongPress: () => _confirmDeleteSleep(session),
      child: Container(
