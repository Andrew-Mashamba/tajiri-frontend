// lib/my_children/pages/baby_dashboard_page.dart
import 'dart:async';
...
class _BabyDashboardPageState extends State<BabyDashboardPage>
  ...
  Widget _buildWeeklyInsightsCard(bool sw) {
    // Only show if daily summary has week data
    final thisWeekFeeds = _dailySummary?['week_feed_count'] as int?;
    final lastWeekFeeds = _dailySummary?['last_week_feed_count'] as int?;
    final thisWeekSleepMin = _dailySummary?['week_sleep_minutes'] as int?;
    final lastWeekSleepMin = _dailySummary?['last_week_sleep_minutes'] as int?;
    final thisWeekDiapers = _dailySummary?['week_diaper_count'] as int?;
    final lastWeekDiapers = _dailySummary?['last_week_diaper_count'] as int?;
    final daysLogged = _dailySummary?['days_logged'] as int? ?? 0;
    final topFeedHours = _dailySummary?['top_feed_hours'] as List<dynamic>?;

    // If backend doesn't provide weekly data, compute from today's data as fallback
    // Show card only if at least 7 days of data
    if (daysLogged < 7 &&
        thisWeekFeeds == null &&
        thisWeekSleepMin == null) {
      return const SizedBox.shrink();
    }

    // Compute comparisons
    final feedDiff = (thisWeekFeeds != null && lastWeekFeeds != null)
        ? thisWeekFeeds - lastWeekFeeds
        : null;
    final sleepDiffMin = (thisWeekSleepMin != null && lastWeekSleepMin != null)
        ? thisWeekSleepMin - lastWeekSleepMin
        : null;
    final diaperDiff = (thisWeekDiapers != null && lastWeekDiapers != null)
        ? thisWeekDiapers - lastWeekDiapers
        : null;
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    sw ? 'Muhtasari wa Wiki' : 'Weekly Insights',
                    style: const TextStyle(
                      fontSize: 15,
                      fontWeight: FontWeight.w700,
                      color: _kPrimary,
            ...
            if (feedDiff != null)
              _buildComparisonRow(
                sw ? 'Kulisha' : 'Feeds',
                '$thisWeekFeeds ${sw ? "wiki hii" : "this week"}',
                feedDiff,
                sw,
              ),
            if (sleepDiffMin != null) ...[
              const SizedBox(height: 8),
              _buildComparisonRow(
                sw ? 'Usingizi' : 'Sleep',
                '${(thisWeekSleepMin! / 60).toStringAsFixed(1)}h ${sw ? "wiki hii" : "this week"}',
                sleepDiffMin,
                sw,
            ...
            if (diaperDiff != null) ...[
              ...
              _buildComparisonRow(
                sw ? 'Nepi' : 'Diapers',
                '$thisWeekDiapers ${sw ? "wiki hii" : "this week"}',
                diaperDiff,
                sw,
              ),
            ],
            ...
            // Pattern detection
            if (topFeedHours != null && topFeedHours.isNotEmpty) ...[
              const SizedBox(height: 12),
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        sw
                            ? 'Kulisha kunafanyika zaidi saa ${topFeedHours.join(" na ")}'
                            : 'Most feeds happen at ${topFeedHours.join(" and ")}',
                        style: const TextStyle(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        sleepDiffMin == 0
                            ? (sw
                                ? 'Usingizi ni sawa na wiki iliyopita'
                                : 'Sleep is the same as last week')
                            : sleepDiffMin > 0
                                ? (sw
                                    ? 'Usingizi umeongezeka kwa ${_fmtMins(sleepDiffMin)} wiki hii'
                                    : 'Sleep improved by ${_fmtMins(sleepDiffMin)} this week')
                                : (sw
                                    ? 'Usingizi umepungua kwa ${_fmtMins(-sleepDiffMin)} wiki hii'
                                    : 'Sleep decreased by ${_fmtMins(-sleepDiffMin)} this week'),
                        style: TextStyle(
                          fontSize: 12,
                          color: sleepDiffMin >= 0
                              ? const Color(0xFF2E7D32)
  ...
  Widget _buildComparisonRow(
    ...
    final color = diff > 0
        ...
        : diff < 0
            ? Colors.red
            : _kSecondary;
    final diffLabel = formatAsDuration
        ? _fmtMins(diff.abs())
        : '${diff.abs()}';
  ...
  Widget _buildHealthIntegrationRow(bool sw) {
    return Row(children: [
      Expanded(child: GestureDetector(
        onTap: () {
          _closeFab();
          Navigator.push(context, MaterialPageRoute(builder: (_) => DoctorModule(userId: widget.userId)));
        },
        child: Container(
          padding: const EdgeInsets.all(14),
      ...
      Expanded(child: GestureDetector(
        onTap: () {
          _closeFab();
          Navigator.push(context, MaterialPageRoute(builder: (_) => PharmacyModule(userId: widget.userId)));
        },
        child: Container(
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: _kCardBg, borderRadius: BorderRadius.circular(12)),
  ...
  Widget _buildShopCard(bool sw) {
    return GestureDetector(
      onTap: () {
        _closeFab();
        Navigator.pushNamed(context, '/home?tab=shop');
      },
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: _kCardBg, borderRadius: BorderRadius.circular(12)),
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 2),
            Text(sw ? 'Tafuta bidhaa za ${widget.baby.name}' : 'Find products for ${widget.baby.name}', style: const TextStyle(fontSize: 12, color: _kSecondary), maxLines: 1, overflow: TextOverflow.ellipsis),
          ])),
  ...
  Widget _buildQuickNavGrid(bool sw) {
    final items = <_NavItem>[
      ...
      _NavItem(Icons.mood_rounded, sw ? 'Meno' : 'Dental', () {
        _closeFab();
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => DentalRecordPage(child: widget.baby)));
      }),
      ...
      _NavItem(Icons.emergency_rounded, sw ? 'Dharura' : 'Emergency', () {
        _closeFab();
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => EmergencyCardPage(child: widget.baby)));
      }),
      _NavItem(Icons.badge_rounded, sw ? 'Kadi ya RCH' : 'Digital RCH Card',
          () {
        _closeFab();
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => RchCardPage(child: widget.baby)));
      }),
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (_) => DoctorSharingPage(
                    child: widget.baby, userId: widget.userId)));
      }),
    ];
  ...
  Widget _buildIncompleteProfilePrompt(bool sw) {
    ...
    if (widget.baby.allergies.isEmpty) missing.add(sw ? 'mzio' : 'allergies');
    if (widget.baby.emergencyContacts.isEmpty) missing.add(sw ? 'mawasiliano ya dharura' : 'emergency contacts');
    if (widget.baby.bloodType == null || widget.baby.bloodType!.isEmpty) missing.add(sw ? 'kundi la damu' : 'blood type');
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 10),
        Expanded(child: Text(
          sw ? 'Kamilisha wasifu wa ${widget.baby.name} — ongeza ${missing.join(", ")}'
             : 'Complete ${widget.baby.name}\'s profile — add ${missing.join(", ")}',
