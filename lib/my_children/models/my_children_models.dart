// lib/my_children/models/my_children_models.dart
...
class Child {
  ...
  factory Child.fromJson(Map<String, dynamic> json) {
    ...
    return Child(
      ...
      schoolId:
          json['school_id'] != null ? _parseInt(json['school_id']) : null,
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (emergencyContacts.isNotEmpty) 'emergency_contacts': emergencyContacts,
  ...
  /// Returns the developmental stage based on age.
  /// infant (0-2), toddler (2-5), school_age (5-12), teen (12-18)
  String get stage {
    final years = ageInYears;
    if (years < 2) return 'infant';
    if (years < 5) return 'toddler';
    if (years < 12) return 'school_age';
    return 'teen';
  }

  /// Bilingual stage label.
  String stageLabel({bool isSwahili = false}) {
    switch (stage) {
      case 'infant':
        return isSwahili ? 'Mtoto Mchanga' : 'Infant';
      case 'toddler':
        return isSwahili ? 'Mtoto Mdogo' : 'Toddler';
      case 'school_age':
        return isSwahili ? 'Mtoto wa Shule' : 'School Age';
      case 'teen':
        return isSwahili ? 'Kijana' : 'Teen';
      default:
        return isSwahili ? 'Mtoto' : 'Child';
    }
  }

  // ─── Age label ──────────────────────────────────────────────

  String get ageLabel => ageLabelLocalized(isSwahili: false);

  String ageLabelLocalized({bool isSwahili = false}) {
    if (isSwahili) {
      if (ageInDays < 7) return 'Siku $ageInDays';
      if (ageInDays < 30) return 'Wiki $ageInWeeks';
      if (ageInMonths < 24) return 'Miezi $ageInMonths';
      return 'Miaka $ageInYears';
    } else {
...
/// Backward-compatible alias so existing code referencing Baby still compiles.
typedef Baby = Child;
...
class CaregiverShare {
  ...
  final String role; // 'co_parent', 'caregiver', 'viewer', 'doctor', 'school'
...
typedef SleepListResult = MyChildrenListResult<SleepSession>;
typedef DiaperListResult = MyChildrenListResult<DiaperLog>;
typedef GrowthListResult = MyChildrenListResult<GrowthMeasurement>;
typedef HealthLogListResult = MyChildrenListResult<HealthLog>;
typedef PhotoListResult = MyChildrenListResult<BabyPhoto>;
typedef CaregiverListResult = MyChildrenListResult<CaregiverShare>;

// ─── Result Wrappers ───────────────────────────────────────────

class MyChildrenResult<T> {
  final bool success;
  final T? data;
  final String? message;
  MyChildrenResult({required this.success, this.data, this.message});
}

class MyChildrenListResult<T> {
  final bool success;
  final List<T> items;
  final String? message;
  MyChildrenListResult({required this.success, this.items = const [], this.message});
}

/// Backward-compatible aliases for existing code.
typedef MyBabyResult<T> = MyChildrenResult<T>;
typedef MyBabyListResult<T> = MyChildrenListResult<T>;
