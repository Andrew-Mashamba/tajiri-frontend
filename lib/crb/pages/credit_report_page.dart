// lib/crb/pages/credit_report_page.dart
...
class CreditReportPage extends StatefulWidget {
  ...
  const CreditReportPage({super.key, required this.businessId, this.standalone = false});
...
class _CreditReportPageState extends State<CreditReportPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final futures = await Future.wait([
        CrbService.getCreditReport(_token!, widget.businessId),
        CrbService.getCreditScore(_token!, widget.businessId),
      ]);
  ...
  Future<void> _requestFreshReport() async {
    ...
    try {
      final res =
          await CrbService.requestCreditReport(_token!, widget.businessId);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(sw ? 'Ripoti ya Mkopo' : 'Credit Report',
  ...
  Widget _buildScoreCard(bool sw) {
    ...
    final grade = _score?.grade ?? _report?.riskGrade ?? 'XX';
