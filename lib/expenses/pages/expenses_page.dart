...
class _ExpensesPageState extends State<ExpensesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final futures = await Future.wait([
        ExpensesService.getExpenses(_token!, widget.businessId,
            category: _selectedCategory?.name),
        ExpensesService.getExpenseSummary(_token!, widget.businessId),
      ]);
  ...
  Future<bool> _confirmDeleteExpense(Expense e) async {
    ...
    final res = await ExpensesService.deleteExpense(_token!, e.id!);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(res.success ? (isSwahili ? 'Matumizi yamefutwa' : 'Expense deleted') : (res.message ?? (isSwahili ? 'Imeshindikana' : 'Failed')))));
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                          ...
                          _topPillActionButton(
                            ...
                            onTap: () async {
                              final added = await Navigator.push<bool>(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => AddExpensePage(businessId: widget.businessId),
                                ),
                              );
                              if (added == true) _load();
                            },
                      ...
                      const SizedBox(height: 12),
                      // Summary card
                      if (_summary != null) _buildSummaryCard(nf, isSwahili),
                      ...
                      SizedBox(
                        ...
                        child: ListView(
                          ...
                          children: [
                            _filterChip(null, isSwahili ? 'Zote' : 'All'),
                            ...ExpenseCategory.values
                                .map((c) => _filterChip(c, _categoryLabel(c, isSwahili: isSwahili))),
                          ],
                        ),
                      ),
                      ...
                      if (_expenses.isEmpty)
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text(isSwahili ? 'Bonyeza "Matumizi Mapya" kuingiza' : 'Use "New Expense" to record an expense',
                                    style: TextStyle(
                        ...
                        ...(_expenses
                            .map((e) => _buildExpenseCard(e, nf, df, isSwahili))),
  ...
  Widget _buildSummaryCard(NumberFormat nf, bool isSwahili) {
    final s = _summary!;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(isSwahili ? 'Matumizi ya Mwezi Huu' : 'This Month\'s Expenses',
              style: TextStyle(
          ...
          Row(
            children: [
              ...
              Text(
                '${s.changePercent.abs().toStringAsFixed(1)}% ${isUp ? (isSwahili ? 'zaidi' : 'more') : isDown ? (isSwahili ? 'pungufu' : 'less') : (isSwahili ? 'sawa na' : 'same as')} ${isSwahili ? 'mwezi uliopita' : 'last month'}',
                style: TextStyle(
                  color: isUp
                      ? Colors.red.shade300
                      : isDown
          ...
          const SizedBox(height: 4),
          Text('${isSwahili ? 'Mwezi uliopita' : 'Last month'}: TZS ${nf.format(s.totalLastMonth)}',
              style: TextStyle(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                      e.description ?? _categoryLabel(e.category, isSwahili: isSwahili),
                      style: const TextStyle(
                          fontWeight: FontWeight.w600,
                          color: _kPrimary,
                          fontSize: 13),
                  ...
                  Row(
                    children: [
                      Text(_categoryLabel(e.category, isSwahili: isSwahili),
                          style: const TextStyle(
            ...
            Column(
              ...
              children: [
                ...
                if (e.paymentMethod != null)
                  Text(_paymentMethodLabel(e.paymentMethod!, isSwahili),
                      style: const TextStyle(
                          fontSize: 10, color: _kSecondary)),
              ],
            ),
  ...
  String _paymentMethodLabel(String method, bool isSwahili) {
    switch (method.toLowerCase()) {
      case 'cash':
        return isSwahili ? 'Taslimu' : 'Cash';
      case 'mpesa':
        return 'M-Pesa';
      case 'airtel':
        return 'Airtel Money';
