// lib/business/pages/create_quote_page.dart
...
class CreateQuotePage extends StatefulWidget {
  ...
  const CreateQuotePage({super.key, required this.businessId});
  ...
  Future<void> _save({bool send = false}) async {
    ...
    if (_items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(sw ? 'Ongeza angalau bidhaa moja' : 'Add at least one item')));
      return;
    }
    final customerName =
    ...
    try {
      final res =
          await BusinessService.createQuote(_token!, widget.businessId, body);
      if (mounted) {
        setState(() => _saving = false);
        if (res.success) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(send
                  ? (sw ? 'Kadirio limetumwa!' : 'Quote sent!')
                  : (sw ? 'Kadirio limehifadhiwa!' : 'Quote saved as draft!'))));
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, scrollCtrl) => ListView(
          ...
          children: [
            ...
            Center(
              child: Text(sw ? 'KADIRIO / ESTIMATE' : 'QUOTE / ESTIMATE',
                  style: const TextStyle(
                      fontSize: 20,
            ...
            Text('${sw ? 'Halali Hadi' : 'Valid Until'}: ${df.format(_validUntil)}',
            ...
            _previewTotalRow(sw ? 'Jumla Ndogo' : 'Subtotal', 'TZS ${nf.format(_subtotal)}'),
            if (_includeVat)
              _previewTotalRow('VAT (${_vatRate.toStringAsFixed(0)}%)',
                  'TZS ${nf.format(_vatAmount)}'),
            const SizedBox(height: 4),
            _previewTotalRow(sw ? 'JUMLA' : 'TOTAL', 'TZS ${nf.format(_totalAmount)}',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(sw ? 'Tuma Kadirio' : 'Send Quote',
                    style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
              ),
            ),
          ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(sw ? 'Unda Kadirio' : 'Create Quote',
            style: const TextStyle(
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: InputDecorator(
                    decoration: InputDecoration(
                      labelText: sw ? 'Halali Hadi' : 'Valid Until',
                      filled: true,
                      fillColor: _kCardBg,
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(10),
                    ...
                    child: Text(df.format(_validUntil)),
                  ),
                ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _previewTotalRow(
                          sw ? 'Jumla Ndogo' : 'Subtotal', 'TZS ${nf.format(_subtotal)}'),
                      if (_includeVat)
                        _previewTotalRow('VAT (18%)', 'TZS ${nf.format(_vatAmount)}'),
                      const Divider(),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                    ...
                    Expanded(
                      child: SizedBox(
                        ...
                        child: OutlinedButton(
                          ...
                          child: Text(sw ? 'Hifadhi Rasimu' : 'Save Draft',
                      ...
                      child: SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          child: _saving
                              ...
                              : Text(sw ? 'Tuma Kadirio' : 'Send Quote',
                                  style: const TextStyle(
