...
This document summarizes agent skills for the TAJIRI platform (maintained under `.cursor/skills/`, aligned with the former Claude Code setup), including their triggers and purposes.
...
Persistent knowledge stored in `.cursor/memory/`:
...
**Note**: Use the matching `SKILL.md` when the topic matches; the agent applies patterns based on task and open files (see `.cursor/rules/`).