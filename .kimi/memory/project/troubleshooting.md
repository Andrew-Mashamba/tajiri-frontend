...
## Agent Destructive Changes — CRITICAL #issue #safety
**Date**: 2026-04-29
**Context**: Background coder agents deleted files and gutted shared models.

**Problem**: Three `Agent(subagent_type="coder", run_in_background=true)` tasks timed out after 30 minutes. Their partial output:
- Deleted `lib/business/pages/payroll_page.dart`, `credit_report_page.dart`, `employees_page.dart`, `purchase_orders_page.dart`, `suppliers_page.dart`
- Gutted `lib/business/pages/tax_page.dart` from ~16 KB to 93 bytes
- Stripped `getClientNotes`, `getClientReminders`, `addClientReminder`, `addClientNote`, `deleteClientNote`, `updateClientNote`, `syncClientsFromShop` from `BusinessService`
- Removed fields from `Invoice`, `InvoiceItem`, `Debt` models (customerPhone, unitLabel, isVatExempt, discountType, discountValue, paymentTerms, etc.)
- Result: 312 compile errors across invoices (209), skincare (31), hair_nails (29), vfd (22), clients (7), debts (6)

**Solution / Recovery**:
```bash
git checkout HEAD -- .
```
This restored all tracked files to last committed state. The three new untracked pages I built survived because they were never in git.

**Prevention**:
- Read `.claude/skills/safety-guardrails/SKILL.md` at every session start.
- Background `coder` agents are BANNED. Only foreground edits.
- Run `flutter analyze` after every batch of changes.
- Grep for callers before touching shared models or services.

**Tags**: #safety #agent #deletion #recovery #critical
