...
## Safety & Agent Control Pattern — MANDATORY #pattern #safety
**Date**: 2026-04-29
**Context**: Background agents caused catastrophic file deletions and model stripping.

**What happened**: Three `coder` agents were launched with `run_in_background=true` to implement F3/F4/F5/F7 items. They timed out after 30 minutes. Their partial work deleted 5 business pages, gutted `tax_page.dart`, stripped methods from `BusinessService`, and broke 312 compile errors across invoices, skincare, hair_nails, vfd, clients, debts.

**Hard rules forever**:
1. **Never delete existing files** without explicit user permission. Mark deprecated, ask first.
2. **Never strip methods/fields/enums** from shared models/services without grep-checking all callers first.
3. **Never launch background coder agents**. All code changes happen in foreground where every edit is reviewed.
4. **Always run `flutter analyze`** after changes. Fix errors before declaring done.
5. **Prefer minimal surgical edits**. One concern per change. No "refactor while you're in there."
6. **Shared files with >3 dependents** get full-project analyze after editing.
7. **Stop immediately** if analyze shows new errors, if >5 files need changing, or if shared APIs change.

**Files**: `.claude/skills/safety-guardrails/SKILL.md`
**Tags**: #safety #agent-control #mandatory #pattern
